...
namespace SA.Transport.Core.Application.Features.Commands
{
    ...
    public class UpdateTransportValidator : AbstractValidator<UpdateTransportRequest>
    {
        public UpdateTransportValidator()
        {
            ...
            RuleFor(x => x.MakeGE)
                .NotNull().WithMessage("მიუთითეთ მარის ქართული დასახელება")
                .MaximumLength(50).WithMessage("გამოიყენეთ მაქსიმუმ 50 სიმბოლო");
            //.Matches(@"S""[\p{IsGeorgian}\p{P}\p{N}\s]*""").WithMessage("მარკის ქართული დასახელება უნდა შედგებოდეს ქართული სიმბოლოებისგან");

            RuleFor(x => x.MakeEN)
               .NotNull().WithMessage("მიუთითეთ მარკის ლათინური დასახელება")
               .MaximumLength(50).WithMessage("გამოიყენეთ მაქსიმუმ 50 სიმბოლო");
            // .Matches(@"S""[\p{IsBasicLatin}\p{P}\p{N}\s]*""").WithMessage("მარკის ლათინური დასახელება უნდა შედგებოდეს ლათინური სიმბოლოებისგან");
